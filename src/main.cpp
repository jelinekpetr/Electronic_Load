...
/*********************************************/
void toggleModeLeft() {
  if (statusRegister[1] > 0) {statusRegister[1] -= 1;}
  else if (statusRegister[1] == 0) {statusRegister[1] = 3;}
  cleared = false;
...
void toggleModeRight() {
  if (statusRegister[1] < 3) {statusRegister[1] += 1;}
  else if (statusRegister[1] == 3) {statusRegister[1] = 0;}
  cleared = false;
  if (!mainScreen._refreshEnabled) {
    mainScreen._refreshEnabled = true;
  }
...
void setup() {
  ...
  mainScreen.addTextField(6, "Vco", 11, 2, 9, "Vc:", "V ", VOLT_VALUE);
